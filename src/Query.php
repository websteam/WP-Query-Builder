...
class Query
{
    ...
    public function insert($data = null)
    {
        ...
        if (!$this->table) {
            throw new \BadMethodCallException("No table set. Please call ->table('tablename') before calling ->insert().");
        }
    ...
    public function update($data = null, $force = false)
    {
        ...
        if (!$this->table) {
            throw new \BadMethodCallException("No table set. Please call ->table('tablename') before calling ->update().");
        }

        if (!$force && $this->wheres->isEmpty()) {
            throw new \BadMethodCallException('No where condition set. Please use updateAll instead.');
        }
    ...
    public function delete($force = false)
    {
        ...
        if (!$this->table) {
            throw new \BadMethodCallException("No table set. Please call ->table('tablename') before calling ->delete().");
        }

        if (!$force && $this->wheres->isEmpty()) {
            throw new \BadMethodCallException('No where condition set. Please use deleteAll instead.');
        }
            ...
            foreach ($results as $result) {
                ...
                if (is_null($column)) {
                    ...
                    if (!array_key_exists($column, $resultValues)) {
                        ...
                        throw new \InvalidArgumentException("Column {$column} not found in returned set. Must be one of {$columns}.");
                    }
    ...
    public function getTotalRowCount()
    {
        if (!$this->countFoundRows) {
            throw new \BadMethodCallException('getTotalRowCount() can only be called if you have called countFoundRows() before executing the query.');
        }
	...
	{
		return $this->buildSqlAndPrepare();
	}
    ...
    protected function buildSetSql()
    {
        if (count($this->updateFields) === 0) {
            throw new \BadMethodCallException('update() can only be called if you have called set() or passed in an array of fields to update.');
        }
